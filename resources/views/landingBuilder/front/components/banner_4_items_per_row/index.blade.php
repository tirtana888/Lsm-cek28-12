@if(!empty($landingComponent) and $landingComponent->enable)
    ...
    @push('styles_top')
        <link rel="stylesheet" href="{{ getLandingComponentStylePath("banner_4_items_per_row") }}">
    @endpush

    <div class="banner-4-items-per-row-section container position-relative mt-40">
        <div class="row">
            @if(!empty($contents['specific_banners']) and is_array($contents['specific_banners']))
                @foreach($contents['specific_banners'] as $bannerData)
                    @if(!empty($bannerData['image']))
                        <div class="col-12 col-md-3 mt-24">
                            <a href="{{ !empty($bannerData['url']) ? $bannerData['url'] : '' }}" target="_blank" class="">
                                <div class="banner-4-items-per-row-section__img-card {{ $useRounded ? 'rounded-24' : '' }}">
