@if(!empty($landingComponent) and $landingComponent->enable)
    ...
    @push('styles_top')
        <link rel="stylesheet" href="{{ getLandingComponentStylePath("banner_2_items_per_row") }}">
    @endpush

    <div class="banner-2-items-per-row-section container position-relative mt-40">
        <div class="row">
                    ...
                    @if(!empty($bannerData['image']))
                        <div class="col-12 col-md-6 mt-24">
                            <a href="{{ !empty($bannerData['url']) ? $bannerData['url'] : '' }}" target="_blank" class="">
                                <div class="banner-2-items-per-row-section__img-card {{ $useRounded ? 'rounded-24' : '' }}">
