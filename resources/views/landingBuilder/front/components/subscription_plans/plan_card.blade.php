...
<div class="subscription-plan-card position-relative">
    ...
    <div class="position-relative z-index-2 d-flex flex-column bg-white rounded-24 p-16 pt-32 w-100 h-100">
        ...
        <h3 class="font-20 font-weight-bold mt-16">{{ $subscribe->title }}</h3>
        <p class="font-16 text-gray-500 mt-4">{{ $subscribe->subtitle }}</p>
                ...
                @if(!empty($subscribeSpecialOffer))
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-end line-height-1">
                            ...
                            <span class="font-16 text-gray-500 ml-12 text-decoration-line-through">{{ handlePrice($subscribe->price, true, true, false, null, true) }}</span>
                        </div>
        ...
        @if(!empty($subscribe->description))
            <div class="mt-16 p-12 rounded-16 bg-gray-100 font-14 text-gray-500">{!! nl2br(truncate($subscribe->description, 120)) !!}</div>
        @endif

        <ul class="my-16">
            <li class="d-flex align-items-center">
                <x-tick-icon class="icons text-success" width="16" height="16"/>
                <div class="ml-4 font-16 text-gray-500">{{ $subscribe->days }} {{ trans('financial.days_of_subscription') }}</div>
            </li>
